# -----------------------------------------------------
# -- Sionna implementation of the MMSE PIC
...
# -- The code is based on the Sionna implementation of their LMMSE detector.
# -----------------------------------------------------
...
class SisoMmsePicDetector(Layer):
    ...
    def soft_symbols(self, llr_a, points_reshaped, batch_size, num_ofdm_symbols, num_effective_subcarriers, num_tx,
        ...
        if self._low_complexity and self._constellation._constellation_type == "qam" and self._constellation.num_bits_per_symbol in [
            ...
            error_var = self._qam_normalization_factor ** 2 * (
                    (c + d) - tf.square(s_real) - tf.square(s_imag))
            ...
            squared_error = tf.math.pow(
                tf.maximum(tf.abs(tf.expand_dims(s_hat, axis=-1) - points_reshaped), self._epsilon), 2)  # tf.pow
    ...
    def call(self, inputs):
        ...
        # mu_i = a_i^H g_i
        # mu = tf.matmul(A_inv, G)
        # mu_i = tf.expand_dims(tf.linalg.diag_part(mu), axis=-1)
        # mu_i = tf.math.real(mu_i)
        ...
        if llr_a is not None:
            ...
            z_i = tf.linalg.matmul(A_inv, y_hat_i_MF) / tf.cast(mu_i,
                                                                dtype=self.dtype)
...
class sisoLowComplexityMfLmmseDetector(SisoMmsePicDetector):
    # pylint: disable=line-too-long
    """
    Soft-Input Soft-Output MF-LMMSE Filter with trainable parameter \alpha (and \beta).
    """
    ...
    def call(self, inputs):
        ...
        if G is None:
            G = tf.linalg.matmul(h_dt_desired_whitened, h_dt_desired_whitened,
                                 adjoint_a=True)
            # y_MF is [batch_size, num_rx, num_ofdm_symbols, num_effective_subcarriers, num_streams_per_rx]
            y_MF = tf.linalg.matmul(h_dt_desired_whitened, y_dt_whitened,
                                    adjoint_a=True)  # Preprocessing: matched filter is complex O(M_R*N_T)       for all OFDM symbols!!
        else:
            y_MF = y
        ...
        if llr_a is None:
            ...
            [s_hat, error_var, log_pC] = self.soft_symbols(llr_a, points_reshaped, batch_size, num_ofdm_data_symbols,        # low complexity soft symbol calculation for gray mapped QAM constellations
                                                           num_effective_subcarriers, num_tx, num_streams)
        ...
        if llr_a is not None:  # i.e. PIC
            ...
            if self._error_var_term == "default":               # analytical NPI expression, high computational complexity
            ...
            elif self._error_var_term == "sinr_heuristic4":  # low-complexity NPI expression that is used in the DUIDD paper
            ...
            elif self._error_var_term == "lmmse":               # LMMSE NPI expression - suboptimal (only optimal if LMMSE)
