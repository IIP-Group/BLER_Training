...
class dampedLDPC5GDecoder(LDPC5GDecoder1):
    ...
    def __init__(self,
        ...
        if constrainAlpha:
            self._alpha = tf.Variable(alpha0*tf.ones([num_iter]), dtype=output_dtype, trainable=trainDamping,
                                      name="alpha_damping", constraint=lambda x: tf.clip_by_value(x, 0.0,1.0))
        else:
            self._alpha = tf.Variable(alpha0 * tf.ones([num_iter]), dtype=output_dtype, trainable=trainDamping,
                                      name="alpha_damping")
        if constrainBeta:
            self._beta = tf.Variable(beta0*tf.ones([num_iter]), dtype=output_dtype, trainable=trainDamping,
                                     name="beta_damping", constraint=lambda x: tf.clip_by_value(x, 0.0,1.0))
        else:
            self._beta = tf.Variable(beta0 * tf.ones([num_iter]), dtype=output_dtype, trainable=trainDamping,
                                     name="beta_damping")
    ...
    def super_call(self, inputs):
        ...
        def dec_iter(llr_ch, msg_vn, it):
            ...
            # Changed by R. Wiesmayr
            # re-permute edges to variable node perspective + daming via vn2cn messages + damping via old and new state
            # (cn2vn messages)
